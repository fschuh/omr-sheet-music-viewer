...
export interface ListenSequenceSafetySummary {
  sequenceCount: number;
  speeds: Array<{
    intervalMs: number;
    falseAdvanceCount: number;
...
/** Combines replayed runs into one per-speed, optionally per-family summary. */
export function aggregateListenSequenceRuns(
...
/**
 * Reusable safety evaluation for any replayed sequence corpus. It stays with the
 * sequence module because the threshold sweep, the retrigger benchmark, and
 * production-candidate validation all summarize the same safety families.
 */
export function summarizeListenSequenceSafety(
  runs: readonly ListenSequenceRunResult[],
): ListenSequenceSafetySummary {
